...
PARENT_RESULT_SHA256 = "6e6b6bc6cae8b75e6a0c3e0df2757be6e7b1cf03d8578d068d8f34decd769a38"
...
def main():
    ap = argparse.ArgumentParser(description="Cumulative same-scope Repair 2 for SysGrid Project View Perfection Slice H/R4 harness compatibility.")
    ap.add_argument("--repo", default=".")
    ap.add_argument("--output", default=None)
    ns = ap.parse_args()
    repo = pathlib.Path(ns.repo).resolve()
    out = pathlib.Path(ns.output).resolve() if ns.output else repo / "SysGrid-Project-View-Perfection-Repair-2-RESULT.zip"
    result_dir = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair2-result-"))
    ...
    receipt = {
        ...
        "failure_signature": "Repair 1 cleared the legacy sentinel and exposed five pre-R4 Slice H harness assumptions: connector identity, inspect/remove flow, zoom control, hidden filter panel, short-bar resize fixture, and noninteractive visual port target locators.",
        "cause_hypothesis": "harness-only R4 compatibility drift; approved product bytes are preserved and the parent full visual-repair proof already passed.",
        "repair": {
            "path": HARNESS,
            "before_git_blob": HARNESS_BEFORE,
            "transformation": "cumulative R4-aware Slice H harness modernization; no product-source change",
    ...
    temp_root = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair2-proof-"))
