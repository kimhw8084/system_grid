    ...
    async with async_session() as session:
        ...
        for idx in range(28):
            ...
            entity = models.ExternalEntity(
                ...
                ownership_mode=random.choice(["InternalTeam", "InternalOperator"]),
                internal_team_id=team_ref.id,
                internal_operator_id=operator_ref.id if operator_ref else None,
                ...
                contacts_json=[{"name": f"Partner Contact {idx + 1}", "email": f"partner{idx + 1}@example.com", "phone": f"+1-555-30{idx:02}"}],
                ...
                dependency_tier=random.choice(["Tier 0", "Tier 1", "Tier 2"]),
                data_classification=random.choice(["Internal", "Restricted", "Confidential"]),
                integration_mode=random.choice(["Push", "Pull", "Bidirectional"]),
                primary_endpoint_url=f"https://partner-{idx + 1}.vendor.local/api",
                secondary_endpoint_url=f"https://partner-{idx + 1}.vendor.local/dr",
                auth_method=random.choice(["Mutual TLS", "Basic", "OAuth2"]),
                protocol_family=random.choice(["HTTPS", "SFTP", "AMQP"]),
            ...
            if idx < 12:
                session.add(models.ExternalEntitySecret(
                    ...
                    secret_type=random.choice(["Password", "Token", "API Key"]),
                    username=f"ext_user_{idx + 1}",
                    password=f"seed-secret-{idx + 1:03}",
                    vault_provider="SeedLocal",
        ...
        for idx, entity in enumerate(external_entities):
            ...
            session.add(models.ExternalLink(
                ...
                port=str(random.choice([22, 443, 8443, 1521])),
                host_or_fqdn=f"{entity.external_key.lower()}.vendor.local",
                path_or_resource=random.choice(["/api/orders", "/telemetry", "/auth", "/files/drop"]),
                network_zone=random.choice(["DMZ", "Partner Edge", "Manufacturing Secure"]),
                transport_security=random.choice(["TLS1.2", "TLS1.3", "IPSec"]),
                link_status=random.choice(["Active", "Planned", "Restricted"]),
