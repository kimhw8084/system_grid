...
PARENT_RESULT_SHA256 = "30d01f16fa2cba6de437e3e690d642661cde281c455e65c0c00825c59b1a29c5"
...
def repaired_schedule_component_bytes(repo: pathlib.Path):
    ...
    for name, old, new in transforms:
        ...
        out = out.replace(old, new, 1)
        applied.append(name)
    required = [b"import { createPortal } from 'react-dom'", b"createPortal(<aside", b"</aside>, document.body)"]
    missing = [item.decode("utf-8", "replace") for item in required if item not in out]
    if missing:
...
def main():
    ap = argparse.ArgumentParser(description="Same-scope Repair 6 for SysGrid Project View Perfection: retain the fully green diagnostic chain except scheduling, portal the narrow schedule dialog to the viewport, and rerun all canonical proof gates.")
    ap.add_argument("--repo", default=".")
    ap.add_argument("--output", default=None)
    ns = ap.parse_args()
    repo = pathlib.Path(ns.repo).resolve()
    out = pathlib.Path(ns.output).resolve() if ns.output else repo / "SysGrid-Project-View-Perfection-Repair-6-RESULT.zip"
    result_dir = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair6-result-"))
    ...
    receipt = {
        ...
        "failure_signature": "Repair 5 diagnostic sweep passed Slice H/G/F/E/D/C, navigation, and readability; scheduling alone failed because the schedule dialog bounding box started at x=-7px on 390x844.",
        "cause_hypothesis": "the schedule dialog is rendered inside the Projects tree while its hardened CSS uses fixed viewport positioning; portaling the modal to document.body removes the transformed/offset containing-block drift without changing schedule semantics, persistence, ARIA, focus restoration, or Escape behavior.",
        "repair": {
            "path": HARNESS,
        ...
        "schedule_dialog_repair": {"path": SCHED_COMPONENT, "before_git_blob": SCHED_COMPONENT_BEFORE, "transformation": "portal scheduling dialog to document.body so fixed narrow-screen geometry is viewport-anchored while preserving existing dialog semantics"},
    ...
    temp_root = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair6-proof-"))
    ...
    try:
        ...
        if blob(proof_repo, SCHED_COMPONENT) != repaired_schedule_blob:
            raise RuntimeError("isolated schedule dialog portal repair blob mismatch")
        ...
        if blob(repo, SCHED_COMPONENT) != repaired_schedule_blob:
            raise RuntimeError("local schedule dialog portal application verification failed")
