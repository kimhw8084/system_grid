...
PARENT_RESULT_SHA256 = "f7632c17c25631a462e1d4acdfe6027cf84e14fb2b7a8b05a749f47ca0a12ac6"
...
def repaired_slice_g_bytes(repo: pathlib.Path):
    ...
    out = replace_exact(src, "slice-g-r4-transient-shells", old, new)
    ...
    for required in [b"project add/edit menu", b"workspace actions menu", b"expanded project info"]:
        if required not in out:
            raise RuntimeError(f"required Slice G R4 invariant missing: {required!r}")
    return out, ["slice-g-r4-transient-shells"]
...
def main():
    ap = argparse.ArgumentParser(description="Same-scope Repair 4 for SysGrid Project View Perfection: retain Slice H 5/5 product closure and modernize stale pre-R4 retained regression harnesses.")
    ap.add_argument("--repo", default=".")
    ap.add_argument("--output", default=None)
    ns = ap.parse_args()
    repo = pathlib.Path(ns.repo).resolve()
    out = pathlib.Path(ns.output).resolve() if ns.output else repo / "SysGrid-Project-View-Perfection-Repair-4-RESULT.zip"
    result_dir = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair4-result-"))
    ...
    receipt = {
        ...
        "failure_signature": "Repair 3 brought the approved Slice H candidate to 5/5 PASS, then the retained regression chain timed out on removed pre-R4 quick-add/jump selectors in Slice G before any local application.",
        "cause_hypothesis": "the product-specific Slice H acceptance is green; current R4 deliberately replaced old quick-add/jump surfaces and inspect/remove Timeline semantics, so retained Slice G, Slice E, and navigation tests must be bound to the current shell without weakening their accessibility, canonical-write, focus, or navigation assertions.",
        "repair": {
            "path": HARNESS,
    ...
    temp_root = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair4-proof-"))
