  ...
  return <div className="space-y-4" data-project-outcome-form="true">
    <section className="rounded-lg border border-blue-500/15 bg-blue-500/[0.035] p-4" data-project-kind-choice="true">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        <button type="button" onClick={() => { setKind('top'); setParentId('') }} aria-pressed={kind === 'top'} className={`rounded-lg border p-3 text-left ${kind === 'top' ? 'border-blue-500/40 bg-blue-500/10' : 'border-white/10 bg-[var(--sg-surface-1)]'}`}><Target size={16} className="text-blue-300" /><b className="mt-2 block text-[12px] text-white">Top-level Project</b><span className="mt-1 block text-[10px] text-slate-500">Independent MVP/outcome counted once in the portfolio.</span></button>
        <button type="button" onClick={() => setKind('subproject')} aria-pressed={kind === 'subproject'} className={`rounded-lg border p-3 text-left ${kind === 'subproject' ? 'border-violet-500/40 bg-violet-500/10' : 'border-white/10 bg-[var(--sg-surface-1)]'}`}><GitBranch size={16} className="text-violet-300" /><b className="mt-2 block text-[12px] text-white">Subproject</b><span className="mt-1 block text-[10px] text-slate-500">A true child outcome with its own owner, done criteria, success measure and dates.</span></button>
      </div>
      {kind === 'subproject' ? <label className="mt-4 block text-[9px] font-black uppercase tracking-widest text-slate-500">Parent Project<select aria-label="Parent Project" value={parentId} onChange={(event) => setParentId(event.target.value)} className="mt-2 w-full rounded-lg border border-white/10 bg-[var(--sg-surface-2)] px-3 py-2.5 text-[11px] normal-case text-white"><option value="">Choose a parent outcome…</option>{candidates.map((project: any) => <option key={String(project.id)} value={String(project.id)}>{project.name || `Project ${project.id}`}</option>)}</select></label> : <div className="mt-4 rounded-lg border border-emerald-500/10 bg-emerald-500/[0.03] px-3 py-2 text-[10px] text-emerald-300">Top-level Projects cannot select a parent and are the only Projects counted as independent portfolio outcomes.</div>}
    </section>

    <section className="grid gap-3 rounded-lg border border-white/10 bg-[var(--sg-surface-1)] p-4 md:grid-cols-2" data-project-outcome-definition="true">
      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">MVP / outcome<textarea aria-label="MVP / outcome" rows={4} value={objective} onChange={(event) => setObjective(event.target.value)} placeholder="What independently valuable outcome exists when this Project succeeds?" className="mt-2 w-full rounded-lg border border-white/10 bg-[var(--sg-surface-2)] px-3 py-2 text-[11px] normal-case leading-5 text-white outline-none focus:border-blue-500/40" /></label>
      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">Definition of done / success measures<textarea aria-label="Definition of done / success measures" rows={4} value={outcomesText} onChange={(event) => setOutcomesText(event.target.value)} placeholder={'One measurable completion criterion per line\nExample: 95% of target users activated'} className="mt-2 w-full rounded-lg border border-white/10 bg-[var(--sg-surface-2)] px-3 py-2 text-[11px] normal-case leading-5 text-white outline-none focus:border-blue-500/40" /></label>
