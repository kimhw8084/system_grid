...
import { Plus, Trash2, Cpu, Package, Key, X, Check, Save, Layers } from 'lucide-react'
...
const ExpansionModal = ({ title, icon: Icon, color, deviceId, resourceType, onClose }: any) => {
  ...
  const addMutation = useMutation({
    mutationFn: async (val: string) => {
      ...
      if (resourceType === 'hardware') body = { model: val, type: 'CPU' }
      if (resourceType === 'secrets') body = { secret_type: 'SSH', encrypted_payload: val }
      
      return fetch(`/api/v1/devices/${deviceId}/${resourceType}`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel w-[500px] p-8 rounded-3xl space-y-6">
          ...
          {data?.map((item: any) => (
            <div key={item.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group">
              <span className="text-xs text-slate-300 font-mono">
                {item.model || item.name || item.secret_type} {item.encrypted_payload ? `(${item.encrypted_payload})` : ''}
              </span>
        ...
        <div className="flex space-x-2">
          ...
          <button onClick={() => { addMutation.mutate(newValue); setNewValue('') }} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest"><Plus size={16}/></button>
        </div>
      </motion.div>
    </div>
  )
...
const AddAssetModal = ({ onClose }: any) => {
  const queryClient = useQueryClient()
  const [formData, setFormData] = useState({ name: '', system: '', model: '', manufacturer: '', os: '', type: 'physical' })

  const addMutation = useMutation({
    mutationFn: async (data: any) => fetch('/api/v1/devices/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel w-[450px] p-8 rounded-3xl space-y-6">
        <h2 className="text-xl font-bold uppercase tracking-tighter">Provision New Asset</h2>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="text-[10px] font-bold text-slate-500 uppercase">Hostname</label>
            <input onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2 mt-1 outline-none focus:border-blue-500/50" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase">System</label>
            <input onChange={e => setFormData({...formData, system: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2 mt-1 outline-none" />
          </div>
          <div>
        ...
        <div className="flex space-x-3 pt-4">
          <button onClick={onClose} className="flex-1 py-3 text-xs font-bold uppercase tracking-widest text-slate-400">Cancel</button>
          <button onClick={() => addMutation.mutate(formData)} className="flex-1 py-3 bg-[#034EA2] text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg shadow-[#034EA2]/30">Provision</button>
        </div>
      </motion.div>
    </div>
  )
...
export default function AssetGrid() {
  ...
  return (
    <div className="h-full flex flex-col space-y-4 relative">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black tracking-tight uppercase">Asset Intelligence</h1>
