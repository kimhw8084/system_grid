...
describe('ServicesReal source contract', () => {
  it('stays on OperationalDataGrid, shared columns, and shared detail routing', () => {
    ...
    expect(source).toMatch(/\bOperationalRowActionMenu\b/)
    expect(source).toMatch(/\bOperationalRowActionButton\b/)
    expect(rowActionMenuSource).toMatch(/\btoggleFavorite\b/)
    expect(rowActionMenuSource).toMatch(/\btoggleWatch\b/)
    expect(rowActionMenuSource).toMatch(/\bOPERATIONAL_ACTION_LABELS\.archive\b/)
    ...
    expect(rowActionMenuSource).not.toMatch(/\bgrid-cols-2\b/)
    expect(rowActionMenuSource).not.toMatch(/\bgrid-cols-3\b/)
