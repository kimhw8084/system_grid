...
export default function ProjectsCommunication() {
  ...
  return <main className="p08-page" data-workspace="projects" data-pv1-projects-route="true" data-p08-communication="true"><ProjectsOfflineNotice online={online} /><Header projectId={projectId} active={route.route} />{route.route === 'updates' ? <UpdatesView projectId={projectId} project={project.data} refresh={refresh} /> : route.route === 'resources' ? <ResourcesView projectId={projectId} project={project.data} refresh={refresh} /> : <ReportsView projectId={projectId} project={project.data} />}<button className="p08-back" onClick={() => navigate(`/projects/${encodeURIComponent(projectId)}/home`)}>Back to Project Home</button></main>
}
