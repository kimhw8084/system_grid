      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 text-[9px] font-bold uppercase tracking-widest text-slate-600">
            Content restores preserve the independent Active / Archived lifecycle.
          </p>
        </div>
                ...
                <div className="mt-4 space-y-2 border-t border-white/5 pt-4">
                  {deltas.map((delta: any, index: number) => (
                    <div key={`${delta.field || delta.label}-${index}`} className="grid grid-cols-[120px_minmax(0,1fr)_18px_minmax(0,1fr)] items-center gap-2 text-[9px] font-bold">
                      ...
                      <span className="truncate rounded-lg bg-black/30 px-2 py-1 text-slate-500" title={formatHistoryValue(delta.before)}>{formatHistoryValue(delta.before)}</span>
                      <ArrowRight size={11} className="text-slate-700" />
                      <span className="truncate rounded-lg bg-black/30 px-2 py-1 text-slate-300" title={formatHistoryValue(delta.after)}>{formatHistoryValue(delta.after)}</span>
                    </div>
                  ))}
                </div>
              )}
