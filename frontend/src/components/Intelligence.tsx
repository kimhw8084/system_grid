...
import { Upload, FileText, CheckCircle2, Clipboard, Download, AlertCircle, RefreshCcw } from 'lucide-react'
...
export default function Intelligence() {
  ...
  const handlePasteImport = () => {
    ...
    const file = new File([blob], 'pasted_registry.csv')
  ...
  return (
    <div className="h-full flex flex-col space-y-8 max-w-5xl mx-auto overflow-y-auto custom-scrollbar pr-4">
      <div className="text-center space-y-2 mt-8">
        <h1 className="text-3xl font-black tracking-tighter uppercase">Intelligent Bulk Ingestion</h1>
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Automated multi-relational infrastructure provisioning</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div 
          ...
          className={`glass-panel p-12 rounded-3xl border-2 border-dashed transition-all flex flex-col items-center justify-center text-center space-y-6 relative overflow-hidden ${
          ...
          <AnimatePresence mode="wait">
            {status === 'uploading' ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center space-y-4">
                ...
                <p className="text-[10px] font-black uppercase text-blue-400 tracking-widest">Parsing Relational Schema...</p>
              </motion.div>
            ) : status === 'success' ? (
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center space-y-4">
                ...
                <p className="text-[10px] font-black uppercase text-emerald-400 tracking-widest">Ingestion Complete</p>
              </motion.div>
            ) : (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-bold uppercase tracking-tighter">Infrastructure Dropzone</h3>
        ...
        <div className="glass-panel p-8 rounded-3xl bg-slate-900/20 border border-white/5 flex flex-col space-y-4">
           ...
           <textarea 
             ...
             className="flex-1 min-h-[140px] bg-slate-950/50 border border-white/5 rounded-xl p-4 text-[10px] font-mono outline-none focus:border-amber-500/30 text-slate-300 custom-scrollbar" 
             placeholder="hostname,system,model,serial_number...&#10;SRV-01,ERP,R740,SN12345..."
           />
           <button 
             onClick={handlePasteImport}
             disabled={status === 'uploading' || !pasteData.trim()}
             className="w-full py-3 bg-amber-600/10 text-amber-400 border border-amber-600/20 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-600/20 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
           >
             Commit Raw Data
           </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-blue-500 bg-slate-900/40">
           ...
           <p className="text-[10px] text-slate-500 leading-relaxed uppercase font-bold">Download the unified schema template supporting HW/SW sub-relations.</p>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-emerald-500 bg-slate-900/40">
           ...
           <p className="text-[10px] text-slate-500 leading-relaxed uppercase font-bold">Automatically maps Hardware (1:N) and Software (1:N) using hostname primary keys.</p>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-amber-500 bg-slate-900/40">
           ...
           <p className="text-[10px] text-slate-500 leading-relaxed uppercase font-bold">Atomic check ensures serial numbers and asset tags remain unique in the registry.</p>
        </div>
      </div>
    </div>
  )
