...
import { LayoutDashboard, Server, Network, Database, Shield, Settings, Search, ServerCrash } from 'lucide-react'
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#020617] text-slate-100 font-sans antialiased">
      ...
      <aside className="w-60 glass-panel border-r border-white/5 rounded-none p-4 flex flex-col z-20">
        <div className="mb-10 px-4 pt-4">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <span className="font-black text-lg tracking-tighter text-white">SYSGRID</span>
          </div>
        ...
        <nav className="flex-1 space-y-1.5">
          ...
          <SidebarItem icon={Shield} label="Audit Logs" active={activeTab === 'audit'} onClick={() => setActiveTab('audit')} />
        </nav>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        {/* Top Header to prevent overlap */}
        <header className="h-16 border-b border-white/5 flex items-center justify-end px-8 z-10 bg-slate-900/50 backdrop-blur-md">
          <div className="relative group">
            ...
            <input 
              type="text" 
              placeholder="Search SYSGRID..." 
              onFocus={() => setSearchFocused(true)}
              onBlur={() => setSearchFocused(false)}
              className="bg-slate-900 border border-white/10 rounded-full py-2 pl-9 pr-4 text-[11px] w-64 focus:w-80 transition-all focus:outline-none focus:border-blue-500/50" 
            />
          </div>
        </header>
