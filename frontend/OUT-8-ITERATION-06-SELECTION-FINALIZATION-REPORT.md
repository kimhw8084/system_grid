...
- Other changes:
  - None for this report-only retry.
...
- Reserved for future workspaces:
  - the same generic selected ID lifecycle helpers.
...
Environment-limited checks may be blocked:
- `npm run typecheck`
- `npm run build`
If blocked:
- mark exact blocker
- write `CLI proof limited`
...
## 9. Zip/output
- This report file exists at repo root.
- The final zip must include this report file.
- Do not upload unless `zipinfo` proves the report file is inside.
