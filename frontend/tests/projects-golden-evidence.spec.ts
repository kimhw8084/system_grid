...
test.describe('Projects execution intelligence evidence', () => {
  test('all management views render inside the candidate-bound golden shell and Monitoring remains unchanged', async ({ page }) => {
    ...
    await expect(page.getByText('Task-weighted execution', { exact: false }).first()).toBeVisible()
    ...
    await expect(page.getByText('Milestone Control Tower', { exact: true })).toBeVisible()
