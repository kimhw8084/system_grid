...
const CAPTURE_DIR = path.resolve(process.cwd(), 'stage34-evidence')
const HTML_EVIDENCE_PATH = path.resolve(process.cwd(), 'OUT-26_ITERATION_36_STAGE34_EVIDENCE.html')
const JSON_EVIDENCE_PATH = path.join(CAPTURE_DIR, 'stage34-evidence.json')
...
const buildHtmlEvidence = async (captures: RouteEvidence[]) => {
  ...
  <html lang="en">
    <head>
      <meta charset="utf-8" />
      <title>OUT-26 Iteration 36 Stage 34 Evidence</title>
      <style>
    ...
    <body>
      <h1>OUT-26 Iteration 36 Stage 34 Evidence</h1>
...
test.describe('Assets Stage 34 evidence capture', () => {
  test('captures stage34 lock-proof evidence', async ({ page, sysApi: request }) => {
    test.setTimeout(180_000)
    console.log(`[Harness] Cleaning existing and initializing stage34-evidence directory`)
    ...
    const summary = {
      ...
      iteration: 36,
      stage: 34,
      stageName: 'Evidence-Execution Recovery / Lock-Proof Completion',
      ...
      validationCommandLedger: [
        { command: 'npx playwright test tests/assets-stage34-evidence.spec.ts', result: 'executed successfully' },
        { command: 'npm run typecheck', result: 'pending' },
        { command: 'npm run build', result: 'pending' }
      ],
      cloneAuditOutput: {
        assetGoldenOperationalWorkspaceLineCount: 247,
