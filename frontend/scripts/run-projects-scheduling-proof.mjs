...
if (!['rehearsal', 'acceptance'].includes(mode)) {
  console.error(`Unsupported scheduling proof mode: ${mode}`)
  process.exit(2)
}
...
try {
  await waitForServer()
  const grep = mode === 'rehearsal' ? '@scheduling-rehearsal' : '@scheduling-acceptance'
  const runner = spawn('npx', ['playwright', 'test', 'tests/projects-scheduling-completion.spec.ts', '--config=playwright.config.ts', '--workers=1', '--grep', grep], { env, stdio: 'inherit' })
