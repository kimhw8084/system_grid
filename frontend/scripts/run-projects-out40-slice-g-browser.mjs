...
const waitForFrontend = async (baseUrl) => {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    try { const response = await fetch(baseUrl, { redirect: 'manual' }); if (response.status < 500) return } catch (error) { lastError = error }
...
try {
  ...
  const byMode = {
    rehearsal: ['playwright', 'test', 'tests/projects-out40-slice-g-accessible-name-audit.spec.ts', '--grep', '@out40-slice-g-rehearsal', '--workers=1'],
    acceptance: ['playwright', 'test', 'tests/projects-out40-slice-g-accessible-name-audit.spec.ts', '--grep', '@out40-slice-g-acceptance', '--workers=1'],
  }
  if (mode === 'regression') {
    const commands = [
    ...
    for (const command of commands) { if (diagnostic) command.push('--trace=on'); await run(executable('npx'), command, { env: browserEnv }) }
  } else {
