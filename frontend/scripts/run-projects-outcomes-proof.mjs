...
if (!['rehearsal', 'acceptance'].includes(mode)) {
  console.error(`Unsupported outcome proof mode: ${mode}`)
  process.exit(2)
}
...
try {
  await waitForServer()
  const grep = mode === 'rehearsal' ? '@outcomes-rehearsal' : '@outcomes-acceptance'
  const runner = spawn('npx', ['playwright', 'test', 'tests/projects-outcomes.spec.ts', '--config=playwright.config.ts', '--workers=1', '--grep', grep], { env, stdio: 'inherit' })
