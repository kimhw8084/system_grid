...
if (!['rehearsal', 'acceptance', 'diagnostic'].includes(mode)) {
  console.error(`Unsupported readability proof mode: ${mode}`)
  process.exit(2)
}
...
const env = {
  ...
  VITE_IDENTITY_MODE: 'development',
  SYSGRID_USER_ID: 'proof_operator',
}
...
const waitForServer = async () => {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    try {
      ...
      last = new Error(`Vite readiness returned ${response.status}`)
    } catch (error) {
      last = error
    }
...
try {
  await waitForServer()
  const grep = mode === 'rehearsal' ? '@readability-rehearsal' : mode === 'diagnostic' ? '@readability-diagnostic' : '@readability-acceptance'
  const pwArgs = ['playwright', 'test', 'tests/projects-readability.spec.ts', '--config=playwright.config.ts', '--workers=1', '--grep', grep]
  const runner = spawn('npx', pwArgs, { env, stdio: 'inherit' })
