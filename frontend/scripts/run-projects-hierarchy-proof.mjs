...
if (!['rehearsal', 'acceptance'].includes(mode)) {
  console.error(`Unsupported hierarchy proof mode: ${mode}`)
  process.exit(2)
}
...
try {
  await waitForServer()
  const grep = mode === 'rehearsal' ? '@hierarchy-rehearsal' : '@hierarchy-acceptance'
  const runner = spawn('npx', ['playwright', 'test', 'tests/projects-hierarchy.spec.ts', '--config=playwright.config.ts', '--workers=1', '--grep', grep], { env, stdio: 'inherit' })
