...
try {
  ...
  const byMode = {
    rehearsal: ['playwright', 'test', 'tests/projects-scheduling-completion.spec.ts', '--grep', '@scheduling-rehearsal', '--workers=1'],
    acceptance: ['playwright', 'test', 'tests/projects-scheduling-completion.spec.ts', '--grep', '@out40-slice-b-acceptance', '--workers=1'],
  }
  ...
  if (mode === 'regression') {
    ...
    if (diagnostic) { scheduling.push('--trace=on'); readability.push('--trace=on') }
    await run(executable('npx'), scheduling, { env: browserEnv })
    await run(executable('npx'), readability, { env: browserEnv })
  } else {
