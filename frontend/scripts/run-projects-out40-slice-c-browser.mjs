...
try {
  ...
  const byMode = {
    rehearsal: ['playwright', 'test', 'tests/projects-out40-slice-c-board-a11y.spec.ts', '--grep', '@out40-slice-c-rehearsal', '--workers=1'],
    acceptance: ['playwright', 'test', 'tests/projects-out40-slice-c-board-a11y.spec.ts', '--grep', '@out40-slice-c-acceptance', '--workers=1'],
  }
  const browserEnv = { ...process.env, PLAYWRIGHT_BASE_URL: baseUrl }
  if (mode === 'regression') {
    const sliceC = ['playwright', 'test', 'tests/projects-out40-slice-c-board-a11y.spec.ts', '--workers=1']
    ...
    for (const command of [sliceC, scheduling, navigation, readability]) {
