...
if (!['rehearsal', 'acceptance'].includes(mode)) {
  console.error(`Unsupported showcase proof mode: ${mode}`)
  process.exit(2)
}
...
try {
  await waitForServer()
  const grep = mode === 'rehearsal' ? '@showcase-rehearsal' : '@showcase-acceptance'
  const runner = spawn('npx', ['playwright', 'test', 'tests/projects-showcase.spec.ts', '--config=playwright.config.ts', '--workers=1', '--grep', grep], { env, stdio: 'inherit' })
