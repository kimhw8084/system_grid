...
RESULT_NAME = "SysGrid-Project-View-Perfection-Repair-9-RESULT.zip"
...
def apply_css(data: bytes) -> bytes:
    ...
    new_root = b".sg-gantt,.sg-projects-frame,.sg-dialog-shade { --sg-page:transparent; --sg-bg:rgba(255,255,255,.035); --sg-raised:rgba(255,255,255,.065); --sg-line:var(--projects-border-subtle,#29354a); --sg-text:var(--text-primary,#e2e8f0); --sg-muted:var(--projects-text-muted,#a8b4c7); --sg-accent:#60a5fa; color:var(--sg-text); font-family:Inter,ui-sans-serif,system-ui,sans-serif; font-size:13px; line-height:1.4 }\n"
    ...
    new_frame = (
        b".sg-projects-frame { display:flex; flex-direction:column; min-height:0; height:100%; width:100%; min-width:0; max-width:none; margin:0; gap:8px; container-type:inline-size; background:transparent!important }\n"
        ...
        b"[data-project-schedule-control-drawer] { background:rgba(255,255,255,.065)!important; backdrop-filter:blur(18px) }\n"
...
test('Project overview reload is quiet, full-width, bright, and cover-first',async({page},info)=>{
 ...
 const brightness=await page.evaluate(()=>{
  ...
  const composite=(fg:RGBA,bg:RGBA):RGBA=>{const a=fg.a+bg.a*(1-fg.a);if(a<=0)return {r:0,g:0,b:0,a:0};return {r:(fg.r*fg.a+bg.r*bg.a*(1-fg.a))/a,g:(fg.g*fg.a+bg.g*bg.a*(1-fg.a))/a,b:(fg.b*fg.a+bg.b*bg.a*(1-fg.a))/a,a}}
  const lum=(c:RGBA)=>{const v=[c.r,c.g,c.b].map(n=>{const x=n/255;return x<=.04045?x/12.92:Math.pow((x+.055)/1.055,2.4)});return .2126*v[0]+.7152*v[1]+.0722*v[2]}
  const color=(selector:string)=>getComputedStyle(document.querySelector(selector) as HTMLElement).backgroundColor
  const bodyRaw=getComputedStyle(document.body).backgroundColor,frameRaw=color('.sg-projects-frame'),coverRaw=color('[data-project-cover="true"]'),outcomeRaw=color('[data-project-outcome-realization="true"]')
  const body=parse(bodyRaw),frame=composite(parse(frameRaw),body),cover=composite(parse(coverRaw),frame),outcome=composite(parse(outcomeRaw),frame)
  return {body:bodyRaw,frame:frameRaw,cover:coverRaw,outcome:outcomeRaw,bodyL:lum(body),frameL:lum(frame),coverL:lum(cover),outcomeL:lum(outcome)}
 })
...
def main():
    ...
    result_root = repo / ".sysgrid-project-view-repair-r9-result"
    ...
    result = {
        ...
        "repair_iteration": 9,
        "objective": "Project View reload/layout/brightness/cover usability correction — composite-surface repair",
        "status": "FAIL",
