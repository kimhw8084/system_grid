...
PARENT_RESULT_SHA256 = "d0807b48453bc4f88872547bab9a736ec3e7d0a5391b0d34a6314d2bbca862db"
...
SCHED_COMPONENT = "frontend/src/components/ProjectsSchedulingCompletion.tsx"
SCHED_COMPONENT_BEFORE = "dc1c2db96a3de0e91fa1f6d29f8ba22abb417efb"
SCHED_IMPORT_OLD = b"import React, { useEffect, useMemo, useRef, useState } from 'react'\n"
SCHED_IMPORT_NEW = b"import React, { useEffect, useMemo, useRef, useState } from 'react'\nimport { createPortal } from 'react-dom'\n"
SCHED_OPEN_OLD = b"    {timelineActive && selectedProject ? <>\n      {open ? <aside"
SCHED_OPEN_NEW = b"    {timelineActive && selectedProject && open ? createPortal(<aside"
SCHED_CLOSE_OLD = b"      </aside> : null}\n    </> : null}\n"
SCHED_CLOSE_NEW = b"      </aside>, document.body) : null}\n"
...
CSS_SCHEDULE_MOBILE_OLD = b".sg-projects-frame [data-project-task-drawer],[data-project-schedule-control-drawer] { top:60px!important; right:8px!important; bottom:8px!important; width:calc(100vw - 16px)!important; max-width:calc(100vw - 16px)!important }"
CSS_SCHEDULE_MOBILE_NEW = b".sg-projects-frame [data-project-task-drawer] { top:60px!important; right:8px!important; bottom:8px!important; width:calc(100vw - 16px)!important; max-width:calc(100vw - 16px)!important } [data-project-schedule-control-drawer] { top:60px!important; left:8px!important; right:8px!important; bottom:8px!important; width:auto!important; max-width:none!important }"

SCHED_SCENARIO_SELECT_OLD = b'<select style={controlStyle} className={inputClass} value={scenarioTaskId}'
SCHED_SCENARIO_SELECT_NEW = b'<select aria-label="Scenario task" style={controlStyle} className={inputClass} value={scenarioTaskId}'
SCHED_SCENARIO_NAME_OLD = b'<input style={controlStyle} className={`${inputClass} mt-2`} placeholder="Scenario name"'
SCHED_SCENARIO_NAME_NEW = b'<input aria-label="Scenario name" style={controlStyle} className={`${inputClass} mt-2`} placeholder="Scenario name"'
SCHED_BASELINE_NAME_OLD = b'<input style={controlStyle} className={inputClass} value={baselineName} onChange={(event) => setBaselineName(event.target.value)} placeholder="Baseline name"'
SCHED_BASELINE_NAME_NEW = b'<input aria-label="Baseline name" style={controlStyle} className={inputClass} value={baselineName} onChange={(event) => setBaselineName(event.target.value)} placeholder="Baseline name"'
SCHED_BASELINE_SELECT_OLD = b'<select style={controlStyle} className={`${inputClass} mt-2`} value={baselineId}'
SCHED_BASELINE_SELECT_NEW = b'<select aria-label="Schedule baseline" style={controlStyle} className={`${inputClass} mt-2`} value={baselineId}'
...
ALLOWED_TRACKED = set(PARENT_PRODUCT_BLOBS) | {HARNESS, SLICE_G, SLICE_E, NAV, SCHED_COMPONENT}
...
def verify_root_state(repo: pathlib.Path, *, expect_harness_before: bool = True):
    ...
    if expect_harness_before:
        ...
        if not (repo / SCHED_COMPONENT).is_file():
            raise RuntimeError(f"schedule component missing: {SCHED_COMPONENT}")
        actual_schedule = blob(repo, SCHED_COMPONENT)
        if actual_schedule != SCHED_COMPONENT_BEFORE:
            raise RuntimeError(f"schedule component preimage mismatch: {actual_schedule} != {SCHED_COMPONENT_BEFORE}")
...
def repaired_css_bytes(repo: pathlib.Path):
    ...
    if out.count(CSS_SCHEDULE_MOBILE_OLD) != 1:
        raise RuntimeError(f"schedule mobile CSS preimage expected exactly once, found {out.count(CSS_SCHEDULE_MOBILE_OLD)}")
    out = out.replace(CSS_SCHEDULE_MOBILE_OLD, CSS_SCHEDULE_MOBILE_NEW, 1)
    ...
    if b"[data-project-schedule-control-drawer] { top:60px!important; left:8px!important; right:8px!important; bottom:8px!important; width:auto!important; max-width:none!important }" not in out:
        raise RuntimeError("schedule mobile edge-anchoring rule missing after repair")
    if b"[data-project-schedule-control-drawer] { top:60px!important; right:8px!important; bottom:8px!important; width:calc(100vw - 16px)!important" in out:
        raise RuntimeError("obsolete schedule 100vw mobile sizing remains after repair")
    return out


def repaired_schedule_component_bytes(repo: pathlib.Path):
    src = (repo / SCHED_COMPONENT).read_bytes()
    if blob_bytes(src) != SCHED_COMPONENT_BEFORE:
        raise RuntimeError(f"schedule component ancestry mismatch: {blob_bytes(src)} != {SCHED_COMPONENT_BEFORE}")
    out = src
    transforms = [
        ("schedule-dialog-create-portal-import", SCHED_IMPORT_OLD, SCHED_IMPORT_NEW),
        ("schedule-dialog-portal-open", SCHED_OPEN_OLD, SCHED_OPEN_NEW),
        ("schedule-dialog-portal-close", SCHED_CLOSE_OLD, SCHED_CLOSE_NEW),
        ("schedule-dialog-scenario-select-name", SCHED_SCENARIO_SELECT_OLD, SCHED_SCENARIO_SELECT_NEW),
        ("schedule-dialog-scenario-name", SCHED_SCENARIO_NAME_OLD, SCHED_SCENARIO_NAME_NEW),
        ("schedule-dialog-baseline-name", SCHED_BASELINE_NAME_OLD, SCHED_BASELINE_NAME_NEW),
        ("schedule-dialog-baseline-select-name", SCHED_BASELINE_SELECT_OLD, SCHED_BASELINE_SELECT_NEW),
    ]
    applied = []
    for name, old, new in transforms:
        count = out.count(old)
        if count != 1:
            raise RuntimeError(f"schedule transform {name!r} expected exactly one preimage, found {count}")
        out = out.replace(old, new, 1)
        applied.append(name)
    required = [
        b"import { createPortal } from 'react-dom'", b"createPortal(<aside", b"</aside>, document.body)",
        b'aria-label="Scenario task"', b'aria-label="Scenario name"',
        b'aria-label="Baseline name"', b'aria-label="Schedule baseline"',
    ]
    missing = [item.decode("utf-8", "replace") for item in required if item not in out]
    if missing:
        raise RuntimeError(f"required viewport-portal invariants missing after schedule repair: {missing}")
    forbidden = [SCHED_OPEN_OLD, SCHED_CLOSE_OLD]
    leftovers = [item.decode("utf-8", "replace") for item in forbidden if item in out]
    if leftovers:
        raise RuntimeError(f"pre-portal schedule mounting remains after repair: {leftovers}")
    return out, applied
...
def main():
    ap = argparse.ArgumentParser(description="Same-scope Repair 7 for SysGrid Project View Perfection: retain the portal, edge-anchor the narrow schedule dialog to the usable viewport, explicitly name portaled schedule controls, and rerun all canonical proof gates.")
    ap.add_argument("--repo", default=".")
    ap.add_argument("--output", default=None)
    ns = ap.parse_args()
    repo = pathlib.Path(ns.repo).resolve()
    out = pathlib.Path(ns.output).resolve() if ns.output else repo / "SysGrid-Project-View-Perfection-Repair-7-RESULT.zip"
    result_dir = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair7-result-"))
    ...
    receipt = {
        ...
        "failure_signature": "Repair 6 kept Slice H at 5/5 PASS but the portaled schedule dialog still measured x=-7px at 390x844 because mobile CSS sized it from 100vw while scrollbar-gutter reduced the usable fixed containing width; Slice G also found the portaled Scenario task select unnamed.",
        "cause_hypothesis": "after portaling, mobile width still used calc(100vw - 16px), which can exceed the usable fixed containing width when scrollbar-gutter is reserved; portaling also moved scenario/baseline controls outside the Projects-root naming decorator. Edge anchoring and direct names remove both boundary dependencies.",
        "repair": {
            "path": HARNESS,
        ...
        "product_repair": {"path": CSS, "before_git_blob": CSS_BEFORE, "transformation": "retain explicit 40px Gantt toolbar targets and replace schedule mobile 100vw sizing with left/right edge anchoring"},
        "schedule_dialog_repair": {"path": SCHED_COMPONENT, "before_git_blob": SCHED_COMPONENT_BEFORE, "transformation": "retain document.body portal and directly name portaled scenario/baseline controls so geometry and accessibility no longer depend on Projects ancestry"},
    ...
    temp_root = pathlib.Path(tempfile.mkdtemp(prefix="sysgrid-project-view-repair7-proof-"))
    ...
    try:
        ...
        repaired_schedule, applied_schedule = repaired_schedule_component_bytes(repo)
        repaired_schedule_blob = blob_bytes(repaired_schedule)
        receipt["product_repair"]["after_git_blob"] = repaired_css_blob
        receipt["schedule_dialog_repair"]["after_git_blob"] = repaired_schedule_blob
        receipt["schedule_dialog_repair"]["applied_transforms"] = applied_schedule
        ...
        write_atomic(proof_repo / SCHED_COMPONENT, repaired_schedule)
        if blob(proof_repo, SCHED_COMPONENT) != repaired_schedule_blob:
            raise RuntimeError("isolated schedule dialog portal/accessibility repair blob mismatch")
        ...
        receipt["stages"].append({
            ...
            "schedule_dialog_after_git_blob": repaired_schedule_blob,
        })
        ...
        write_atomic(repo / SCHED_COMPONENT, repaired_schedule)
        if blob(repo, SCHED_COMPONENT) != repaired_schedule_blob:
            raise RuntimeError("local schedule dialog portal/accessibility application verification failed")
