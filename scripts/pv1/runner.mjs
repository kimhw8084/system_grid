...
export async function buildChecks({ repoRoot, outputDir }) {
  ...
  const checks = [
    ...
    command('backend:project-domain', 'real_database_api', backend, python, ['-m', 'pytest', '-q', ...backendProjectPaths], { timeoutMs: 1_200_000 }),
    ...
    command('component:projects', 'component_state', frontend, vitest, ['run', ...projectTestPaths], { timeoutMs: 900_000 }),
    ...
    command('accessibility:out40', 'accessibility', frontend, node, ['scripts/run-projects-out40-slice-h-browser.mjs', '--mode', 'regression'], { timeoutMs: 1_200_000 }),
